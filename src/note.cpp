...
static const byte overtone_offsets[] = {12, 18, 24};
...
void start_note(byte chan, byte note, byte volume) {
  ...
  const byte octave_adr[] = {0x10, 0x11,
                             0x12}; // The 3 octave addresses (was 10, 11, 12)
  const byte channel_adr[] = {
      0x08, 0x09, 0x0A,
      0x0B, 0x0C, 0x0D}; // Addresses for the channel frequencies
  byte addressOut;
  byte dataOut;

  // Shift the note down by 1, since MIDI octaves start at C, but octaves on the
  // SAA1099 start at B
  note += 1;

  byte octave = (note / 12) - 1; // Some fancy math to get the correct octave
  byte noteVal =
      note - ((octave + 1) * 12); // More fancy math to get the correct note

  // skip octave setting if same as before
  if (output_status[chan].prevOctaves != octave) {
    // if new octave set it here
    output_status[chan].prevOctaves =
        octave; // Set this variable so we can remember /next/ time what octave
                // was /last/ played on this channel
    // set octave address
    ...
    if (chan == 0 || chan == 2 || chan == 4) {
      dataOut =
          octave |
          (output_status[chan + 1].prevOctaves
           << 4); // Do fancy math so that we don't overwrite what's already on
                  // the register, except in the area we want to.
    }
    if (chan == 1 || chan == 3 || chan == 5) {
      dataOut = (octave << 4) |
                output_status[chan - 1]
                    .prevOctaves; // Do fancy math so that we don't overwrite
                                  // what's already on the register, except in
                                  // the area we want to.
    }
  ...
  // code to treat velocity as volume, reduce upper bound so that attack doesn't
  // take us past max
  byte vol = (volume / 8);
  if (vol <= 0) {
    vol = 1;
...
void handle_note_on(byte channel, byte pitch, byte velocity) {
  ...
  for (byte j = 0; j < sizeof(overtone_offsets); j++) {
    if (!is_channel_free())
      break;
    channel_out = get_channel_out();
    output_status[channel_out].channelActive = true;
    output_status[channel_out].keyOn = true;
    output_status[channel_out].currentPitch = pitch + overtone_offsets[j];
    start_note(channel_out, pitch + overtone_offsets[j], velocity);
  }
}

void handle_note_off(byte channel, byte pitch, byte velocity) {
  for (int i = 0; i < 6; i++) {
    if (output_status[i].currentPitch == pitch && output_status[i].keyOn) {
      output_status[i].keyOn = false;
    }
    for (byte j = 0; j < sizeof(overtone_offsets); j++) {
      if (output_status[i].currentPitch == pitch + overtone_offsets[j] &&
          output_status[i].keyOn) {
        output_status[i].keyOn = false;
      }
    }
  }
...
short int get_channel_out() {
  ...
  int quietest = 0;
  for (int i = 1; i < 6; i++) {
    if (output_status[i].lastVolume < output_status[quietest].lastVolume)
      quietest = i;
  }
  stop_note(quietest);
  return quietest;
...
void proc_attack(short int i) {
  // ATTACK PROCESSING
  if (output_status[i].channelActive == true &&
      output_status[i].attackCount < 4) {
    ...
    if (output_status[i].sinceOn >= attack_rate) {
      ...
      byte dataOut = (output_status[i].lastVolume << 4) |
                     output_status[i].lastVolume; // write new volume
...
void proc_decay(short int i) {
  ...
  if (output_status[i].channelActive == true &&
      output_status[i].attackCount >= 4 &&
      output_status[i].attackCount < 8) { // last number sets sustain
...
void proc_release(short int i) {
  ...
  if (output_status[i].keyOn == false &&
      output_status[i].channelActive == true) {
    output_status[i].sinceOff++;

    if (output_status[i].sinceOff >= release_rate) {
      output_status[i].lastVolume--;

      if (output_status[i].lastVolume <=
          0) { // if release takes us to 0 stop note
        stop_note(i);
      } else {
        byte dataOut = (output_status[i].lastVolume << 4) |
                       output_status[i].lastVolume; // write new volume
